...
class e4FunctionalProgSpec extends FunSpec with Matchers {
  ...
  describe("Projections") {
    import e4FunctionalProg.Projections._
    import project.projections.dao.LocalClient

    describe("sample events") {
      val events = LocalClient.getEvents("src/main/resources/projections/sample.json").get
    ...
    describe("full events") {
      val events = LocalClient.getEvents("src/main/resources/projections/full.json").get
      describe("numberOfEvents") {
        it("should count the number of events") {
