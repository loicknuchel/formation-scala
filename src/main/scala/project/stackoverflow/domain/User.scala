...
case class User(account_id: AccountId,
                user_id: UserId,
                display_name: String,
                user_type: UserType,
                is_employee: Boolean,
                ...
                last_modified_date: Option[Timestamp]) extends Entity {
  val id: String = user_id.toString
}

case class UserEmbed(user_id: UserId,
                     user_type: UserType,
                     display_name: String,
                     profile_image: Link,
                     link: Link,
                     reputation: Int,
                     accept_rate: Option[Int]) extends Entity {
  val id: String = user_id.toString
}
